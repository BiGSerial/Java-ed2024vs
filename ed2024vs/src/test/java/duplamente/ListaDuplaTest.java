...
import com.ed.duplamente.ListaDupla;
...
public class ListaDuplaTest {
    ...
    public void testPartirAoMeio() {
        ...
        assertEquals(3, lista.getQuantNos());
        assertEquals(2, segundaMetade.getQuantNos());

        // Verificar se os elementos estão corretamente distribuídos entre as duas
        // metades
        assertEquals(1, lista.getPrim().getInfo().getChave());
        assertEquals(3, lista.getPrim().getProx().getInfo().getChave());
        assertEquals(5, lista.getUlt().getInfo().getChave());
        assertEquals(2, segundaMetade.getPrim().getInfo().getChave());
        assertEquals(4, segundaMetade.getUlt().getInfo().getChave());
    }
