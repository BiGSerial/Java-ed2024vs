...
public class BlocoPrincipalFilaCircular {
	...
	private static void exibirMensagens(FilaCircular fila, int tamanhoMaximo) {
		if (fila.getTamanho() == tamanhoMaximo) {
			System.out.println(RED + "A fila está cheia.\" + RESET);
		} else if (fila.eVazia()) {
			System.out.println(YELLOW + "A fila está vazia." + RESET);
		} else {
			System.out.println(GREEN + "A fila não está cheia nem vazia." + RESET);
		}
	}
}
