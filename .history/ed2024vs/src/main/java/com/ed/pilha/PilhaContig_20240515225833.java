...
public class PilhaContig {
	...
	public void inverterPilha() {
        if (eVazia() || topo == 1) {
            // Se a pilha estiver vazia ou conter apenas um elemento, não há necessidade de inverter
            return;
        }

        // Cria uma pilha auxiliar para armazenar os elementos temporariamente
        PilhaContig auxiliar = new PilhaContig(this.info.length);

        // Transfere os elementos da pilha original para a pilha auxiliar, invertendo a ordem
        while (!eVazia()) {
            auxiliar.empilhar(desempilhar());
        }

        // Transfere os elementos da pilha auxiliar de volta para a pilha original
        while (!auxiliar.eVazia()) {
